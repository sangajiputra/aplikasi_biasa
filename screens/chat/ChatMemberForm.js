...
import { FlatList, Image, LayoutAnimation, ToastAndroid, TouchableNativeFeedback, View } from "react-native";
...
export class ChatMemberForm extends React.Component {
  state = {
    searchQuery: "",
    users: [],
    page: 1,
  ...
  next(){
    if(this.state.selectedUsers.length === 0){
      ToastAndroid.show("Please invite at least 1 users to the group", 20000);
      return;
    }

    this.props.navigation.navigate("create_chat.form", { users: this.state.selectedUsers });
  }
...
export default connect(mapStateToProps)(ChatMemberForm);
